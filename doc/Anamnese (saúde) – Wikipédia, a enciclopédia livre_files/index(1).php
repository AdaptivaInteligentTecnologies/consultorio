/**
 * @source: [[:en:MediaWiki:Gadget-ReferenceTooltips.js]] ([[en:Special:PermaLink/508635914]])
 * @author: [[:en:User:Yair rand]]
...
window.pg || $(document).ready( function($) {
    ...
    if ( $.inArray(mw.config.get('wgNamespaceNumber'), [ 0, 102, 4, 12 ] ) !== -1 ) {
        function toggleRT(o){
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("Habilitar dicas para as referências").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("Desabilitar dicas para as referências").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").text("Uma vez desabilitadas, as dicas para as referências podem ser reativadas por meio de um link no final da página."),
                                    $("<hr>"),
                                    $("<label>").text("Tempo de espera antes de exibir as dicas para as referências (em milisegundos): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    $("<br>"),
                                    $("<span>").text("Dicas para as referências são ativadas:"),
                                    $("<label>").append(
                            ...
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"Opções das dicas para as referências",buttons:{"Salvar Configurações":function(){
            ...
            $(this)[ isTouchscreen ? 'click' : 'hover' ](function( e ){
                    ...
                    showTimer = setTimeout( function() {
                            ...
                            if(!tooltipNode){
                                    ...
                                    $( tooltipNode.firstChild.insertBefore( document.createElement( "span" ), tooltipNode.firstChild.firstChild ) ).addClass("RTsettings").attr("title", "Configurações das dicas para as referências").click(function(){
