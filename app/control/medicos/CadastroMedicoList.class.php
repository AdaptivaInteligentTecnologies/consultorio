...
/*        $titulo = new TElement('h2');
  ...
  */      
        ...
        $id = new TEntry('med_id');
        $id->setValue(TSession::getValue('session_listagem_cadastro_medico_id'));
        ...
        $this->form->setFields(array($id, $medNome, $find_button, $new_button));
        ...
        $id     = new TDataGridColumn('med_id',    'ID', 'right');
        $medNome   = new TDataGridColumn('med_nome', 'Nome', 'left');


        // add the columns to the DataGrid
        $this->datagrid->addColumn($id);
        $this->datagrid->addColumn($medNome);


        // creates the datagrid column actions
        $order_id = new TAction(array($this, 'onReload'));
        $order_id->setParameter('order', 'med_id');
        $id->setAction($order_id);

        $order_medNome= new TAction(array($this, 'onReload'));
        $order_medNome->setParameter('order', 'med_nome');
        $medNome->setAction($order_medNome);



        // inline editing
        $medNome_edit = new TDataGridAction(array($this, 'onInlineEdit'));
        $medNome_edit->setField('med_id');
        $medNome->setEditAction($medNome_edit);
    ...
    function onSearch()
    {
        ...
        TSession::setValue('session_listagem_cadastro_medico_id_filter',   NULL);
        TSession::setValue('session_listagem_cadastro_medico_nome_filter',   NULL);
        
        TSession::setValue('session_listagem_cadastro_medico_id', '');
        ...
        if ( $data->id )
        {
            ...
            $filter = new TFilter('med_id', '=', "{$data->id}");
            
            // stores the filter in the session
            TSession::setValue('session_listagem_cadastro_medico_id_filter',   $filter);
            TSession::setValue('session_listagem_cadastro_medico_id', $data->id);
        }
        if ( $data->med_nome )
        {
            // creates a filter using what the user has typed
